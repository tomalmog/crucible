...
function defaultExportDir(path: string): string {
  if (!path) return "./outputs/export";
  const base = path.split("/").pop()?.replace(/\.(pt|pth|bin|safetensors)$/, "") ?? "model";
  return `./outputs/export/${base}-huggingface`;
}

export function HfExportForm() {
  ...
  function onModelChange(path: string) {
    setModelPath(path);
    setOutputDir(defaultExportDir(path));
  }
  ...
  async function submit() {
    ...
    try {
      ...
      const label = jobLabel("hf-export", modelPath);
