...
Once your model is trained and evaluated, Crucible helps you package and prepare it for production inference.
...
\`\`\`bash
crucible export --format onnx --model <path> --output <path>
\`\`\`
...
Reduce model size and speed up inference with post-training quantization. Crucible supports **int8** and **int4** quantization, which can shrink model size by 2-4x with minimal quality loss. Quantized models load faster, use less memory, and run significantly quicker on both CPU and GPU.
...
Crucible bundles these checks into the \`crucible verify\` command so you can run them as a single pre-deployment step.
`,
};
