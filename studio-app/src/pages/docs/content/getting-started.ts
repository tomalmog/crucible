...
Point Crucible at a directory of training data. It accepts CSV, JSONL, Parquet, and plain text files.

\`\`\`bash
crucible ingest --source ./my-data --name my-dataset
\`\`\`

Crucible auto-detects the file format, validates the records, and stores everything in a versioned \`.crucible/\` directory.

### 3. Run Your First Training

\`\`\`bash
crucible train --dataset my-dataset --output-dir ./outputs
\`\`\`
...
- **Fine-tune a pre-trained model** — try SFT to adapt an existing model to your instructions: \`crucible train --method sft --model <base-model> --dataset my-dataset\`
- **Explore Crucible Studio** — launch the desktop UI for visual dataset browsing, training configuration, and live monitoring
